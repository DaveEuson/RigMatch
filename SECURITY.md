...
You can expect an acknowledgment within 48 hours. If confirmed, a fix will be released as soon as practical and you will be credited (unless you prefer to remain anonymous).
...
- Makes outbound connections only to these hosts, and only for the stated purpose:
  ...
  - `huggingface.co`, and the Hugging Face download servers under `hf.co` that it redirects to — image and video model files, **only when you press Download and agree to the model's terms**. A download must start at `huggingface.co`, and every redirect it follows must stay on those two domains or it is refused. Each file is checked against its published size and, where Hugging Face publishes one, its SHA-256 before ComfyUI can see it; a file that fails the check twice is deleted rather than kept. Opening a model's license from the download dialog also goes to `huggingface.co`.
