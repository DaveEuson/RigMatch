...
import { AudioLines, Code2, Image as ImageIcon, Maximize2, Play, RefreshCw, Sparkles, X } from 'lucide-react';
  ...
  return (
    <div className="modal-backdrop" role="presentation">
      ...
      >
          ...
          <div className="demo-result-tabs" role="tablist">
            {demos.map((entry, entryIndex) => {
              ...
              const kindTag = entry.kind === 'app' ? 'app'
                ...
                : entry.kind === 'audio' ? 'sound'
                : 'image';
              const KindIcon = entry.kind === 'app' ? Play
                : entry.kind === 'code' ? Code2
                : entry.kind === 'audio' ? AudioLines
                : ImageIcon;
              return (
                <button
        ...
        ) : demo.kind === 'app' && demo.html && demoPreviewUrl ? (
          <iframe
        ...
        ) : demo.kind === 'audio' && demo.audioRef ? (
          <div className="demo-audio-result">
            <AudioClipPlayer audioRef={demo.audioRef} label={`Play the clip ${getShortModelName(demo.model)} made`} />
          </div>
