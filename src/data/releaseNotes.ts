...
export const releaseNotes: ReleaseNoteEntry[] = [
  {
    ...
    notes: [
      ...
      'Four things that said nothing now say what they are. The Balance fader names both ends instead of being a slider with a number on it. Each contestant card says who made the model — "by Google", with the country badge the Models table already used — and says nothing at all where the maker is genuinely unknown. A model already downloaded wears a green On your PC badge instead of gray small print beside a bright download size. The dialog when you close RigMatch is one row per choice, with what it deletes and how much it frees, under "Free up disk space?" instead of a shouted warning, and it takes "Don\'t ask me again".',
      'Two smaller things. The app stopped singing at the fader, and the share card is the contestant\'s. The winner\'s jingle fired on any rise in the Top Match score, and the Balance fader re-ranks what is already measured, so dragging it sang every few notches; a jingle needs a run behind it now. And the card you post is the model\'s own portrait in a gold hairline, where the centerpiece used to be a yellow heart typed from whatever emoji font the reading platform had — a second typeface beside the display type, and it said nothing about which model won. Both card styles carry the portrait, and the Scorecard style is warm plum rather than the blue-black it was, a color no theme in the app uses.',
      'RigMatch is written in one English. A button said "Summarising…" in an app whose own design notes banned "cool greys" and whose code spelled them gray. 145 files moved to American spelling, from this page down to the comments. Two British spellings stay, because they are names the browser and HTML own rather than words of ours.',
    ],
  },
  {
    version: '0.8.0',
