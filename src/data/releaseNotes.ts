...
export const releaseNotes: ReleaseNoteEntry[] = [
  {
    ...
    notes: [
      ...
      'Answer quality is measured more honestly, and in one place it now refuses to answer at all. RigMatch grades every answer by matching a shape — valid JSON with the right keys, a refusal where one belongs, a list of the length you asked for. Everyday chat and writing have no such shape, and what the app had been doing instead was scoring those answers by their LENGTH: a fixed number for any reply, rising with the character count. That is not a measurement of quality, and it was being used to crown "Best for talking" — so the wordiest model won. It no longer crowns anything. Those goals stay uncrowned, and say so, until you switch on the judge in Settings, which reads the answer and marks it properly. Everything the rules can genuinely grade — JSON, refusals, formatting, and the one coding question with a known right answer — is unchanged.',
