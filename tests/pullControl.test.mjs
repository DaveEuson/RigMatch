...
test('cancel outranks pause, in either arrival order', () => {
  ...
  assert.equal(requestPull('cancel', 'pause'), 'cancel', 'pause must not revive a canceled queue');
});
...
test('a pause is only a pause while something is downloading', () => {
  ...
  // the other way round announces a paused queue as canceled and throws the
  ...
  assert.equal(pullOutcome({ request: 'cancel', hasActiveModel: true }), 'canceled');
  assert.equal(pullOutcome({ request: 'cancel', hasActiveModel: false }), 'canceled');
});

test('a queue that nobody stopped reports the real failure', () => {
  // Reporting 'canceled' here would hide a bad tag or a full disk behind a
